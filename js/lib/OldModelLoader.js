...
import { DoubleSide, DynamicDrawUsage, InstancedMesh, MeshStandardMaterial, Object3D, TextureLoader } from "./three.module.js";

/*
Instancing solution based on this one: https://githubmemory.com/repo/troisjs/trois/issues/76
*/

export default class OldModelLoader {
    ...
    loadRocks(density) {
        ...
        loader.load(
            ...
            (object) => {
                ...
                // Instance populating
                for (let i = 0; i < instances; i++){
                    dummy.position.set(pxArr[i], pyArr[i] -0.8, pzArr[i])
                    dummy.rotation.set(0, Math.random() * 2, 0);
                    let scale = Math.random() * 1 + 0.5;
                    dummy.scale.set(scale, scale, scale);
                    dummy.updateMatrix();

...
export default class Skybox extends ShaderMaterial{
    constructor()
    {
        ...
        const fragmentShader =/*glsl*/ `
            ...
            float getSunPoint(vec3 pos, vec3 y){
                return smoothstep(0.03, 0.026, distance(pos, y)) * 50.0;
            }

            float rayleighMultiplier(vec3 pos, vec3 y){
                return 1.0 + pow(1.0 - clamp(distance(pos, y), 0.0, 1.0), 2.0) * PI * 0.5;
            }

            float getMie(vec3 pos, vec3 y){
                float disc = clamp(1.0 - pow(distance(pos, y), 0.1), 0.0, 1.0);
                
                return disc * disc * disc * 2.0 * PI; // se på effektivisering.
            }

            // Fargelegger skybox
            vec3 atmosphericScattering(vec3 pos, vec3 y){
                // Original
                //vec2 correctedY = y / max(iResolution.x, iResolution.y) * iResolution.xy;
                
                vec3 correctedY = y / max(vPosition.x, max(vPosition.y, vPosition.z)) * vPosition;
                ...
                vec3 sun = getSunPoint(pos, correctedY) * absorption;
                vec3 mie = getMie(pos, correctedY) * sunAbsorption;
